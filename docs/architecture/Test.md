...
* **GitOps:** **Argo CD (selected)**. (Flux is an optional future alternative; standardize on Argo CD for this project.)
...
## Tooling matrix (agent‑ready, local → prod)

| Layer                  | Tool (selected / alternative)       | Primary purpose / what it’s for                                         | Local dev?               | AI‑agent control                     | UI available?                     | Notes / lock‑in                                      |
| ---------------------- | ----------------------------------- | ----------------------------------------------------------------------- | ------------------------ | ------------------------------------ | --------------------------------- | ---------------------------------------------------- |
| IaC                    | **Terraform**                       | Provision infra (nets, DNS, LB, registries, clusters), GitOps bootstrap | **Yes (core)**           | CLI (`terraform`), Plan/Apply via CI | No (CLI‑first)                    | Remote state w/ locking; no secrets in state         |
| IaC (orchestration)    | Terragrunt (opt.)                   | DRY orchestration of many TF modules                                    | Optional                 | CLI                                  | No                                | Helpful for multi‑env; keep simple at start          |
| VM/OS config           | Ansible (opt.)                      | VM bootstrap/cloud‑init fallbacks                                       | Optional                 | CLI                                  | No                                | Only if TF can’t cover host config                   |
| Infra via K8s          | Crossplane (opt.)                   | Manage infra via CRDs/GitOps                                            | Optional                 | kubectl/GitOps                       | No (via Argo)                     | Powerful, adds complexity; consider later            |
| GitOps                 | **Argo CD (selected)**              | Desired state sync from Git                                             | **Yes**                  | REST API/CLI, Git PRs                | **Yes**                           | Use App‑of‑Apps + sync‑waves                         |
| Progressive delivery   | Argo Rollouts (opt.)                | Canary/Blue‑Green                                                       | Optional                 | CRDs via GitOps                      | Plugin UI                         | Risk‑managed releases                                |
| CI/CD                  | **GitHub Actions**                  | Build/test/sign/scan/release                                            | Optional (local runners) | REST API                             | Web UI                            | Alt: GitLab CI; avoid Azure DevOps to reduce lock‑in |
| K8s CLI                | **kubectl**                         | Operate/debug clusters                                                  | **Yes**                  | CLI                                  | No                                | Guard with context checks                            |
| Context switch         | kubectx/kubens                      | Safe context/namespace switching                                        | **Yes**                  | CLI                                  | No                                | Prevent wrong‑cluster ops                            |
| Packaging              | **Helm**                            | Deploy charts (platform/apps)                                           | **Yes**                  | CLI/GitOps                           | No                                | Pin chart versions                                   |
| Kustomization          | kustomize                           | Overlays/patches                                                        | **Yes**                  | GitOps                               | No                                | Keep base vs overlays clean                          |
| Scripting              | Task / Make                         | Uniform local/CI commands                                               | **Yes**                  | CLI                                  | No                                | Single entrypoints for agents                        |
| Local cluster          | **kind**                            | Fast local K8s for dev/CI                                               | **Yes**                  | CLI                                  | No                                | Version‑pinned; map 80/443                           |
| On‑prem cluster        | **k3s / RKE2 / kubeadm**            | On‑prem K8s                                                             | No (prod)                | CLI/Ansible                          | No                                | Choose one; k3s easiest                              |
...
| Backup/DR              | **Velero**                          | Backup/restore + snapshots                                              | **Yes**                  | CLI/GitOps                           | Minimal UI                        | Use S3‑API (MinIO/Azure/GCS)                         |
